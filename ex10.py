...
plt.plot(
    myRho, -1, marker="^", color="darkorange", linestyle="none", label=r"True $\rho_G$"
)
plt.ylim([-1.5, -0.95])
plt.xlim([0, 1])
...
print(
    "95\%% CI\t\t\t\t [%0.2f,%0.2f]"
    % (np.maximum(rhoHat - 1.96 * se, 0), np.minimum(rhoHat + 1.96 * se, 1))
)
# =============================================
...
numModels = 6
x0 = np.array(
    [[0.1, 0.2], [-1, 0.7], [-1, 0.7], [0.48, 0.005], [-1, 0.7], [0.48, 0.005]]
)
...
for n in range(0, numModels):
    mixPara[n, :] = scipy.optimize.fsolve(
        ac.mixtureMethodOfMoment, x0[n, :], args=(dHat, vHat, n)
    )
    M1[n], M2[n] = mix.calibrateVerify(
        mixPara[n, 0], mixPara[n, 1], dHat, 0.015, mOrder[n]
    )
...
for n in range(0, numModels):
    print(
        "%s\t %0.2f\t %0.2f\t %0.2f\t %0.2f\t %0.2f\t %0.2f\t %0.3f"
        % (
            mList[n],
            1e4 * dHat,
            1e4 * M1[n],
