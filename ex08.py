...
for s in range(0, S):
    ...
    for n in range(0, N):
        num = np.dot(
            testIS[n, varIndicator], np.multiply(rnDerivative[varIndicator], c[n])
        )
...
for n in range(0, nView):
    print(
        "%d\t %d\t %0.1f\t %0.1f\t %0.1f"
        % (n + 1, loc[n], VaRC[loc[n]], mcVaRC[loc[n]], isVaRC[loc[n]])
    )
print("--------------------------------------------------------")
print(
    "Total\t\t %0.1f\t %0.1f\t %0.1f" % (np.sum(VaRC), np.sum(mcVaRC), np.sum(isVaRC))
)
print("Generating 99% CONFIDENCE INTERVALS")
varCheck = np.zeros([len(alpha), S])
esCheck = np.zeros([len(alpha), S])
for s in range(0, S):
    elCheck, ulCheck, varCheck[:, s], esCheck[:, s] = th.oneFactorThresholdModel(
