...
def myApprox(l, p, c, p1, p2, whichModel, myDegree, constant=0, den=1):
    ...
    if myDegree == 2:
        constant = l
        den, err = nInt.quad(
            computeYIntegral, lB, uB, args=(l, p, c, p1, p2, whichModel, 1)
        )
    num, err = nInt.quad(
        computeYIntegral, lB, uB, args=(l, p, c, p1, p2, whichModel, myDegree)
    )
...
def myVaRCY(l, p, c, p1, p2, whichModel):
    ...
    for n in range(0, len(p)):
        num[n], err = nInt.quad(
            varCNumerator, lB, uB, args=(l, n, p, c, p1, p2, whichModel)
        )
...
def esCVaR(y, l, myN, p, c, p1, p2, whichModel, myAlpha, extraTerm=0):
    ...
    t_l = getSaddlePoint(pY, c, l)
    baseTerm = np.divide(
        pY[myN] * np.exp(c[myN] * t_l), computeMGF(t_l, pY[myN], c[myN])
    )
    if t_l < 0:
...
def myESCY(l, p, c, p1, p2, whichModel):
    ...
    for n in range(0, len(p)):
        esC[n], err = nInt.quad(
            integrateAll, lB, uB, args=(l, n, p, c, p1, p2, whichModel)
        )
...
def myApproxT(l, p, c, p1, p2, whichModel, myDegree, constant=0, den=1):
    ...
    if myDegree == 2:
        constant = l
        den, err = nInt.nquad(
            computeYIntegralT, support, args=(l, p, c, p1, p2, whichModel, 1)
        )
    num, err = nInt.nquad(
        computeYIntegralT, support, args=(l, p, c, p1, p2, whichModel, myDegree)
    )
    return constant + np.divide(num, den)
...
def myVaRCYT(l, p, c, p1, p2, whichModel):
    ...
    num = np.zeros(len(p))
    for n in range(0, len(p)):
        num[n], err = nInt.nquad(
            varCNumeratorT, support, args=(l, n, p, c, p1, p2, whichModel)
        )
    return c * np.divide(num, den)
...
def myESCYT(l, p, c, p1, p2, whichModel):
    ...
    esC = np.zeros(len(p))
    for n in range(0, len(p)):
        esC[n], err = nInt.nquad(
            integrateAllT, support, args=(l, n, p, c, p1, p2, whichModel)
        )
    return (1 / myAlpha) * esC
