...
for n in range(0, nView):
    print(
        ...
        % (n + 1, loc[n], indVaRC[loc[n]], gVaRC[loc[n]], wVaRC[loc[n]], tVaRC[loc[n]])
    )
print("---------------------------")
print(
    "Total\t\t %0.1f\t %0.1f\t %0.1f\t %0.1f"
    % (np.sum(indVaRC), np.sum(gVaRC), np.sum(wVaRC), np.sum(tVaRC))
)
...
plt.bar(
    ind + margin + (3 * width), rVaRC[:, 3], color="blue", width=width, label=r"$t$"
)
...
ahat = scipy.optimize.minimize_scalar(
    vc.findAlphaGaussian,
    0.9999,
...
myAlpha = ahat.x
mcMatchedRisk, varMatched, esMatched = vc.mcThresholdGDecomposition(
    N, M, S, p, c, rhoG, 0, 0, myAlpha
)
...
print("--------------------------------------------------------")
print(
    "Total\t\t %0.1f\t %0.1f\t %0.1f\t %0.1f\t %0.1f"
    % (np.sum(gVaRC), np.sum(varMC), np.sum(varMatchedC), np.sum(esCG), np.sum(esMC))
)
