...
el[0], ul[0], var[:, 0], es[:, 0], cTime[0] = irb.runModelSuite(
    N, M, p, c, alpha, nu, rhoG, rhoTarget, tenor, 4
)
print("Running ASRF MODEL")
el[1], ul[1], var[:, 1], es[:, 1], cTime[1] = irb.runModelSuite(
    N, M, p, c, alpha, nu, rhoG, rhoTarget, tenor, 5
)
...
for n in range(0, len(alpha)):
    GA[n] = irb.granularityAdjustmentCR(
        myA, irb.getW(myP, myA, rhoG, alpha[n]), gBar, xi, p, c, alpha[n]
    )
# =====================
...
for n in range(0, len(alpha)):
    print(
        ...
        % (1e2 * alpha[n], var[n, 0], var[n, 1], var[n, 2], var[n, 0] + GA[n])
    )
...
plt.plot(
    var[:, 0] + GA, alpha, color="gray", linestyle=":", label="IRB + GA", linewidth=3
)
plt.xlabel("USD")
plt.ylabel("Quantile")
plt.legend(loc=4)
...
for n in range(0, N):
    ...
    gaPart = irb.gaContribution(
        myA, irb.getW(myP, myA, rhoG, 0.9999), gBar, xi, p, c, n, 0.9999
    )
