...
def bivariateTCdf(yy, xx, rho, nu):
    t_ans, err = nInt.dblquad(
        bivariateTDensity, -10, xx, lambda x: -10, lambda x: yy, args=(rho, nu)
    )
    return t_ans
...
def calibrateMFT(B, pMean, regionId, nu):
    myBounds = ((0.001, 0.30), (0.001, 0.30), (0.001, 0.30), (0.001, 0.30))
    M = 100
    ...
    xhat = scipy.optimize.minimize(
        calibrateOF,
        xStart,
        args=(B, pMean, regionId, nu),
        method="SLSQP",
        jac=None,
        bounds=myBounds,
    )
...
def thresholdCalibrationGridSearch(dGrid, myP, rhoTarget, whichModel, nu=30):
    ...
    for n in range(0, len(dGrid)):
        ...
        if whichModel == 1:
            ...
            jointDefaultProb[1, n], err = nInt.nquad(
                bivariateGDensity, support, args=(dGrid[n], 2)
            )
        elif whichModel == 2:
            support = [[-8, myT.ppf(myP, nu)], [-8, myT.ppf(myP, nu)]]
            jointDefaultProb[0, n] = jointDefaultProbabilityT(myP, myP, dGrid[n], nu)
            jointDefaultProb[1, n], err = nInt.nquad(
                bivariateTDensity, support, args=(dGrid[n], nu)
            )
        dEstimate[0, n] = np.divide(jointDefaultProb[0, n] - myP ** 2, myP * (1 - myP))
        dEstimate[1, n] = np.divide(jointDefaultProb[1, n] - myP ** 2, myP * (1 - myP))
    print(
        "The conditonal approach gives %0.2f"
        % (np.interp(rhoTarget, dEstimate[0, :], dGrid))
    )
    print(
        "The classic approach gives %0.2f"
        % (np.interp(rhoTarget, dEstimate[1, :], dGrid))
    )
    return dEstimate
...
def jointDefaultProbabilityNVM(p, q, myRho, myA, whichModel):
    ...
    support = [[-8, 8], [0, 100]]
    pr, err = nInt.nquad(
        jointIntegrandNVM, support, args=(p, q, myRho, myA, invCdf, whichModel)
    )
    return pr
