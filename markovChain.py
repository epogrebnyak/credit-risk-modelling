...
def getPremiumAccrual(beta, fRate, tenor, h, Delta, d):
    myAccrual = 0
    for i in range(0, beta):
        sIncrement = survivalProb(i * Delta, tenor, h) - survivalProb(
            (i + 1) * Delta, tenor, h
        )
        myAccrual += (
            fRate
            * (Delta / 2)
            * np.interp(i * Delta + (Delta / 2), tenor, d)
            * sIncrement
        )
    return myAccrual
...
def getProtectionPayment(beta, fRate, tenor, h, Delta, d, R):
    ...
    for i in range(0, beta):
        ...
        myProtection += (
            (1 - R) * np.interp(i * Delta + (Delta / 2), tenor, d) * sIncrement
        )
    return myProtection
