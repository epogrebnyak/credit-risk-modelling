...
def logitProbitBinomialAnalytic(N, c, mu, sigma, alpha, isLogit):
    ...
    for k in range(0, N + 1):
        pmf[k], err = nInt.quad(
            logitProbitMixtureFunction, 0, 1, args=(N, k, mu, sigma, isLogit)
        )
    cdf = np.cumsum(pmf)
...
def poissonMixtureAnalytic(N, myC, a, b, alpha, whichModel):
    ...
    for k in range(0, N + 1):
        pmfMixture[k], err = nInt.quad(
            poissonMixtureIntegral, 0, k + 1, args=(k, a, b, N, whichModel)
        )
...
def poissonMixtureCalibrate(x, pTarget, rhoTarget, whichModel):
    if x[1] <= 0:
        return [100, 100]
    M1, err = nInt.quad(
        poissonMixtureMoment, 0.0001, 0.9999, args=(x[0], x[1], 1, whichModel)
    )
    M2, err = nInt.quad(
        poissonMixtureMoment, 0.0001, 0.9999, args=(x[0], x[1], 2, whichModel)
    )
    f1 = pTarget - M1
    f2 = rhoTarget * (M1 - (M1 ** 2)) - (M2 - (M1 ** 2))
    return [f1, f2]
...
def poissonMixtureCalibrate1(x, pTarget, rhoTarget, whichModel):
    if x[1] <= 0:
        return [100, 100]
    M1, err = nInt.quad(
        poissonTransformedMixtureMoment,
        0.0001,
        0.9999,
        args=(x[0], x[1], 1, whichModel),
    )
    M2, err = nInt.quad(
        poissonTransformedMixtureMoment,
        0.0001,
        0.9999,
        args=(x[0], x[1], 2, whichModel),
    )
    f1 = pTarget - M1
    f2 = rhoTarget * (M1 - (M1 ** 2)) - (M2 - (M1 ** 2))
    return [f1, f2]
...
def calibrateVerify(a, b, targetP, targetRho, model):
    ...
    print(
        "Target default probability is %0.3f and target default correlation is %0.3f"
        % (targetP, targetRho)
    )
    print(
        "Calibrated default probability is %0.3f and calibrated default correlation is %0.3f"
        % (M1, defCorr(M1, M2))
    )
    return M1, M2
