...
plt.bar(
    np.linspace(0, K - 1, K) - 0.35,
    1e2 * numberOfJumps / (N * (T + 1)),
    color="cornflowerblue",
)
plt.xticks(np.linspace(0, K - 1, K).astype(int), np.linspace(0, K - 1, K).astype(int))
...
print(
    "The average error is %0.2f" % (1e2 * np.mean(np.abs(P[0:-1, :] - Phat[0:-1, :])))
)
...
plt.bar(
    np.linspace(1, K ** 2 - K, K ** 2 - K) - 0.40,
    x[0:-K],
    yerr=myT.ppf(1 - 0.05 / 2, T - 1) * seAnl,
    color="grey",
    label="ML estimates",
)
plt.plot(
    np.linspace(1, K ** 2 - K, K ** 2 - K),
    xTrue[0:-K],
    marker="*",
    color="red",
    linestyle="None",
...
plt.xticks(np.linspace(1, K ** 2 - K, K ** 2 - K).astype(int), myLabels)
