...
# (a) Calibrate
kTarget = 6
vgModel = scipy.optimize.fsolve(
    th.nvmCalibrate, np.array([0.2, 1]), args=(myP, rhoTarget, kTarget, 0)
)
rhoVG = vgModel[0]
vgA = vgModel[1]
...
# (b) Simulate
startTime = time.time()
el[2], ul[2], var[:, 2], es[:, 2] = th.oneFactorNVMModel(
    N, M, p, c, rhoVG, vgA, alpha, 0
)
...
for n in range(0, len(alpha)):
    print(
        ...
        % (1e2 * alpha[n], var[n, 0], var[n, 1], var[n, 2])
    )
