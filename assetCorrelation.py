...
def getProdCMF(g, myRho, myP, myN, myK):
    pg = th.computeP(myP, myRho, g)
    return np.multiply(
        util.getBC(myN, myK), np.power(pg, myK) * np.power(1 - pg, myN - myK)
    )
...
def logL2r(x, T, pMat, nMat, kMat):
    ...
    for t in range(0, T):
        L[t], err = nInt.quad(
            getCMF2r, -5, 5, args=(x, pMat[t, :], nMat[t, :], kMat[t, :])
        )
...
def mapRatingData(Y, D, K):
    ...
    for n in range(0, N):
        for m in range(1, T):
            d_low[n, m - 1], d_upp[n, m - 1] = mapRating(
                D, Y[n, m - 1].astype(int), Y[n, m].astype(int), K
            )
    return d_low, d_upp
...
def mixtureMethodOfMoment(x, myP, myV, myModel):
    if myModel == 0:  # Beta-binomial
        ...
        M1, err = nInt.quad(
            mix.poissonMixtureMoment, 0.0001, 0.9999, args=(x[0], x[1], 1, 0)
        )
        M2, err = nInt.quad(
            mix.poissonMixtureMoment, 0.0001, 0.9999, args=(x[0], x[1], 2, 0)
        )
    elif myModel == 5:  # Poisson-Weibull
        M1, err = nInt.quad(
            mix.poissonMixtureMoment, 0.0001, 0.9999, args=(x[0], x[1], 1, 1)
        )
        M2, err = nInt.quad(
            mix.poissonMixtureMoment, 0.0001, 0.9999, args=(x[0], x[1], 2, 1)
        )
    f1 = M1 - myP
    f2 = (M2 - M1 ** 2) - myV
...
def getThresholdMoments(x, myP, whichModel):
    if whichModel == 0:  # Gaussian
        ...
        M1, err = nInt.nquad(
            thresholdMoment, support, args=(x[0], x[1], myP, whichModel, 1)
        )
        M2, err = nInt.nquad(
            thresholdMoment, support, args=(x[0], x[1], myP, whichModel, 2)
        )
    elif whichModel == 2:  # Variance-gamma
        invCdf = th.nvmPpf(myP, x[1], 0)
        support = [[-7, 7], [0, 100]]
        M1, err = nInt.nquad(
            thresholdMoment, support, args=(x[0], x[1], myP, whichModel, 1, invCdf)
        )
        M2, err = nInt.nquad(
            thresholdMoment, support, args=(x[0], x[1], myP, whichModel, 2, invCdf)
        )
...
def logLSimple_cr(x, T, pVec, nVec, kVec, myA):
    ...
    for t in range(0, T):
        L[t], err = nInt.quad(
            getCMF_cr, 0, 100, args=(x, myA, pVec[t], nVec[t], kVec[t])
        )
    return -np.sum(np.log(L))
...
def getRegionalDefaultCorrelation(rhoVec, myP):
    ...
    return np.divide(
        jp - myP[0] * myP[1],
        np.sqrt(myP[0] * (1 - myP[0])) * np.sqrt(myP[1] * (1 - myP[1])),
    )
